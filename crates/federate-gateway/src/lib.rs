...
fn styled_error(status: StatusCode, title: &str, detail: &str) -> Response {
    // Official Federate font. When the failing domain can still serve blocks
    // (e.g. a 404 path on a healthy site) the woff2 loads; otherwise the
    // browser falls back to Georgia gracefully.
    let html = format!(
        r##"<!doctype html><html><head><meta charset="utf-8"><title>{title}</title>
<style>
@font-face{{font-family:'Averia Serif Libre';font-style:normal;font-weight:400;font-display:swap;src:url('/fonts/averia-serif-libre-400.woff2') format('woff2')}}
@font-face{{font-family:'Averia Serif Libre';font-style:normal;font-weight:700;font-display:swap;src:url('/fonts/averia-serif-libre-700.woff2') format('woff2')}}
