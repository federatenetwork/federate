...
| federate-mutation | signed mutation envelopes, signed audit events, RegistryBackend trait + RedbRegistryStore (embedded db), persistent RegistryStore apply path, persistent nonces, seed-file init, JSON-to-redb migration + backup/restore | real; 32 tests incl. native-protocol e2e |
...
2. **Public deployment: DONE for one node.** Node 1 runs live on a Hetzner VPS at 195.201.171.223 (Docker containers behind the box's existing Traefik: native 4077 public, DNS on the public IP:53, gateway behind a priority-1 catch-all on port 80), externally verified: native handshake, full pinned-key chain fetch of the runtime-published home.fed, port-80 browser door, upstream DNS forwarding. Registry backed up daily via cron. federate.network now serves the bootstrap API over Let's Encrypt TLS (the CLI's default bootstrap URL works with zero flags) and DNS-over-HTTPS at /dns-query, which defeats port-53 interception (verified from an intercepting network). Client onboarding is one command: `curl -fsSL https://federate.network/install.sh | bash` (macOS/Linux) or `iex (irm https://federate.network/install.ps1)` (Windows) installs a prebuilt CLI and runs `federate setup`: a local verifying resolver (`federate dns proxy`, the same DnsServer engine on 127.0.0.1:53 as launchd/systemd/schtasks service; TLD set comes from the continuously refreshed signed root zone, so no client-side TLD list exists), system DNS pointed at it with exact restore on uninstall, dynamic cleanup of stale hosts-file entries (any hosts name the signed zone resolves is removed, backup kept), fed:// handler registration, and a live resolve+fetch self-test. Remaining: TLS for Federate names themselves (the gateway door is still plain HTTP per host). **Done.**
...
7. **TLS story for Federate names: DONE for installed machines.** `federate setup` creates a per-machine local CA (mkcert model: key generated on-device, never shared, added to the system trust store, removed on uninstall) and the resolver service runs a loopback verifying gateway (http 127.0.0.1:80 + https 127.0.0.1:443) with per-SNI leaf certificates minted at first use; Federate DNS answers point at 127.0.0.1, so browsers get `https://home.fed` with a green lock while every byte is chain-verified on the machine itself. Remote gateways (non-installed visitors) remain plain HTTP by design. Tested: CA idempotence + rustls-verified leaf chain + full browser-grade SNI handshake e2e. **Done for the installed path.**
